...
class MatchResult:
    ...
    guest_total_match = ''
    guest_total_get_goal = ''
    guest_total_loss_goal = ''
    guest_total_score = ''
...
def conn():
    connect = pymssql.connect(server='192.168.90.76', user='tdms_user', password='tdms_user@123!', database='TDMS_SY') #服务器名,账户,密码,数据库名
    if connect:
        print("连接成功!")
    return connect
...
def getdetail(url):
    ...
    matchResult.guest_all_total_get_goal_rate = round(
        float(matchResult.guest_all_total_get_goal) / float(matchResult.guest_all_total_match), 2)
    matchResult.guest_all_total_loss_goal_rate = round(
    ...
    matchResult.guest_total_get_goal_rate = round(
        float(matchResult.guest_total_get_goal) / float(matchResult.guest_total_match), 2)
    matchResult.guest_total_loss_goal_rate = round(
...
def savetoDB(matchResultList):
    sql = 'insert into ball(match_day,seq,league_title,home_team,guest_team,score,result,home_all_total_match,home_all_total_get_goal,\
    home_all_total_loss_goal,home_all_total_score,home_all_total_get_goal_rate,home_all_total_loss_goal_rate,\
    home_total_match,home_total_get_goal,home_total_loss_goal,home_total_score,home_total_get_goal_rate,\
    home_total_loss_goal_rate,guest_all_total_match,guest_all_total_get_goal,guest_all_total_loss_goal,\
    guest_all_total_score,guest_all_total_get_goal_rate,guest_all_total_loss_goal_rate,guest_total_match,\
    guest_total_get_goal,guest_total_loss_goal,guest_total_score,guest_total_get_goal_rate,guest_total_loss_goal_rate) \
    values(%s, %s, %s, %s, %s, %s,%d, %d, %d, %d, %d, %s, %s, %d, %d, %d, %d, %s, %s, %d, %d, %d, %d, %s, %s, %d, %d, %d, %d, %s, %s)'

    data = []
    for matchResult in matchResultList:
        data .append((matchResult.match_day, matchResult.seq, matchResult.league_title
                      , matchResult.home_team, matchResult.guest_team, matchResult.score
                      , matchResult.result, matchResult.home_all_total_match, matchResult.home_all_total_get_goal
                      , matchResult.home_all_total_loss_goal, matchResult.home_all_total_score, matchResult.home_all_total_get_goal_rate
                      , matchResult.home_all_total_loss_goal_rate, matchResult.home_total_match
                      , matchResult.home_total_get_goal, matchResult.home_total_loss_goal, matchResult.home_total_score
                      , matchResult.home_total_get_goal_rate, matchResult.home_total_loss_goal_rate, matchResult.guest_all_total_match
                      , matchResult.guest_all_total_get_goal, matchResult.guest_all_total_loss_goal, matchResult.guest_all_total_score
                      , matchResult.guest_all_total_get_goal_rate, matchResult.guest_all_total_loss_goal_rate, matchResult.guest_total_match
                      , matchResult.guest_total_get_goal, matchResult.guest_total_loss_goal, matchResult.guest_total_score
                      , matchResult.guest_total_get_goal_rate, matchResult.guest_total_loss_goal_rate))
    #print(data)
    con = conn()
    cursor = con.cursor()
...
def get_match_to_predict(year, month, day):
    ...
    for i in range((end - begin).days + 1):
        ...
        for tr in soup.find_all(has_attr_cansale):
            ...
            home_all_total_get_goal_rate = matchResult.home_all_total_get_goal_rate
            home_all_total_loss_goal_rate = matchResult.home_all_total_loss_goal_rate

            home_rate = round(float(matchResult.home_total_score) / float(matchResult.home_total_match), 2)
            home_total_get_goal_rate = matchResult.home_total_get_goal_rate
            home_total_loss_goal_rate = matchResult.home_total_loss_goal_rate

            guest_all_rate = round(float(matchResult.guest_all_total_score) / float(matchResult.guest_all_total_match), 2)
            guest_all_total_get_goal_rate = matchResult.guest_all_total_get_goal_rate
            guest_all_total_loss_goal_rate = matchResult.guest_all_total_loss_goal_rate

            guest_rate = round(float(matchResult.guest_total_score) /  float(matchResult.guest_total_match), 2)
            guest_total_get_goal_rate = matchResult.guest_total_get_goal_rate
            guest_total_loss_goal_rate = matchResult.guest_total_loss_goal_rate
...
def main():
    #get_result(2020, 6, 16, 2020, 6, 18)
    get_match_to_predict(2020, 6, 19)
