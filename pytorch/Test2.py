...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Linear(84, 102) # 定义输出为10，因为10各类
    def forward(self, x):
...
def main():
    ...
    # train_data = MyDataset(txt=root + 'train.csv', transform=transforms.ToTensor())
    train_data = MyDataset(txt=root + 'train.csv', transform=transform)
    ...
    test_data = MyDataset(txt=root + 'test.csv', transform=transform)
    test_loader = DataLoader(dataset=test_data, batch_size=4, shuffle=True, num_workers=3)
    print('num_of_testData:', len(test_loader))

    predict_data = TestDataset(txt=root + 'predict.csv', transform=transform)
    predict_loader = DataLoader(dataset=predict_data, batch_size=1, shuffle=True, num_workers=1)
    print('num_of_predictData:', len(predict_loader))

    #net = Net()
    net = torchmd.densenet201(True)
    net.features = 102
